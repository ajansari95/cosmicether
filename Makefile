...
ifeq (cleveldb,$(findstring cleveldb,$(COSMICETHER_BUILD_OPTIONS)))
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=cosmicether \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=cosmicetherd \
...
ifeq (,$(findstring nostrip,$(COSMICETHER_BUILD_OPTIONS)))
  ldflags += -w -s
endif
ldflags += $(LDFLAGS)
...
draw-deps:
	...
	@goviz -i ./cmd/cosmicetherd -d 2 | dot -Tpng -o dependency-graph.png
...
docker-build-debug:
	@docker build -t cosmos/cosmicetherd-e2e -f e2e.Dockerfile .
...
start-localnet-ci: build
	rm -rf ~/.cosmicetherd-liveness
	./build/cosmicetherd init liveness --chain-id liveness --home ~/.cosmicetherd-liveness
	./build/cosmicetherd config chain-id liveness --home ~/.cosmicetherd-liveness
	./build/cosmicetherd config keyring-backend test --home ~/.cosmicetherd-liveness
	./build/cosmicetherd keys add val --home ~/.cosmicetherd-liveness
	./build/cosmicetherd add-genesis-account val 10000000000000000000000000stake --home ~/.cosmicetherd-liveness --keyring-backend test
	./build/cosmicetherd gentx val 1000000000stake --home ~/.cosmicetherd-liveness --chain-id liveness
	./build/cosmicetherd collect-gentxs --home ~/.cosmicetherd-liveness
	sed -i.bak'' 's/minimum-gas-prices = ""/minimum-gas-prices = "0uatom"/' ~/.cosmicetherd-liveness/config/app.toml
	./build/cosmicetherd start --home ~/.cosmicetherd-liveness --x-crisis-skip-assert-invariants
